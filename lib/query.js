...
module.exports = function(knex, tableName, fields, joins, queries, onChange) {
  ...
  query.orderByAfter = function(column, last) {
    // If no last value, go straight to order by
    if (last == undefined) return this.orderBy(column);

    if (column[0] == '-') { // DESC
      var field = column.substr(1);
      return this.where(field, '<', last).orderBy(column);
    } else { // ASC
      return this.where(column, '>', last).orderBy(column);
    }
  }
