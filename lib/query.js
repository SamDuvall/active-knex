...
module.exports = function(knex, tableName, fields, joins, queries, events) {
  ...
  var deferreds = [];
  query.defer = function(callback) {
    deferreds.push(callback);
    return this;
  }

  // Map the result of a select statement, so the fields are to our liking
  function mapResult(result) {
    var method = query._method || 'select';
  ...
  query.then = function(onFulfilled, onRejected) {
    return Promise.each(deferreds, function(deferred) {
      ...
      return table.then.call(query, function(result) {
