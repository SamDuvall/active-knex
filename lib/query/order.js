...
function extendQuery(qb, schema) {
  ...
  qb.afterId = function(order, lastId) {
    ...
    // Determine join tables
    var joins = _.map(qb.joined, function(joined) {
      if (joined.from === schema.tableName) return joined.to;
      return [joined.from, joined.to].join('.');
    });
    ...
    // Run the search and then add to order
    return qb.defer(function() {
      var findQb = schema.query();
      findQb.where(idColumn, lastId);
      findQb.joins.apply(findQb, joins);
