...
function extendQuery(qb, schema) {
  ...
  var afterWhereTpl = _.template('(<%= column %> <%= sign %> <%= value %> OR (<%= column %> = <%= value %> AND id <%= sign %> <%= id %>))');
  ...
  qb.after = function(column, lastValue, lastId) {
    ...
    if (lastValue !== undefined && lastId !== undefined) {
      ...
      qb = qb.whereRaw(where);
    }

    // Set the ordering
    var direction = reverse ? 'desc' : 'asc';
    var idColumn = schema.tableName + '.id';
    return qb.orderBy(column, direction).orderBy(idColumn, direction);
  };
